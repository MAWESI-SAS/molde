...
fn parse_entity_inner(src: &str) -> Result<Table> {
    ...
    let header = nodes
        .first()
        .ok_or_else(|| EfmError::new(0, "archivo de entidad vacío"))?;
    if nodes.len() > 1 {
        return Err(EfmError::new(
            nodes[1].line,
        ...
        for st in &sub.children {
            ...
            for part in split_top_level(inner.trim_matches(|c| c == '{' || c == '}'), ',') {
                ...
                let (fname, frest) = part
                    .split_once(':')
                    .ok_or_else(|| EfmError::new(st.line, "campo de subtipo inválido"))?;
...
fn parse_field_parts(
    ...
    if tokens.is_empty() {
        return Err(EfmError::new(line, format!("campo '{name}' sin tipo")));
    }
    ...
    if tokens[0] == "owns" {
        ...
        for part in split_top_level(inner, ',') {
            ...
            let (oname, orest) = part
                .split_once(':')
                .ok_or_else(|| EfmError::new(line, "campo owned inválido"))?;
...
fn parse_column(
    ...
    for f in facets {
        if f == "pk" {
            ...
            col.max_length = Some(
                v.parse()
                    .map_err(|_| EfmError::new(line, format!("maxlen inválido: '{v}'")))?,
            );
            ...
            let p = it
                ...
                .ok_or_else(|| EfmError::new(line, format!("precision inválida: '{v}'")))?;
            col.precision = Some(p);
            if let Some(s) = it.next() {
                col.scale = Some(
                    s.trim()
                        .parse()
                        .map_err(|_| EfmError::new(line, format!("scale inválida: '{s}'")))?,
                );
            }
            ...
            return Err(EfmError::new(line, format!("faceta desconocida: '{f}'")));
        }
    }
...
fn parse_key(table_name: &str, node: &Node) -> Result<PrimaryKey> {
    ...
    if columns.is_empty() {
        return Err(EfmError::new(node.line, "bloque 'key:' sin columnas"));
    }
...
fn parse_fk(table_name: &str, node: &Node) -> Result<ForeignKey> {
    ...
    let map = obj
        .as_object()
        .ok_or_else(|| EfmError::new(node.line, "se esperaba un objeto en belongs-to"))?;

    let columns =
        value_strlist(map.get("fk")).ok_or_else(|| EfmError::new(node.line, "FK sin 'fk'"))?;
    let refs = map
        .get("references")
        .and_then(value_str)
        .ok_or_else(|| EfmError::new(node.line, "FK sin 'references'"))?;
...
fn parse_references(s: &str, line: usize) -> Result<(Option<String>, String, Vec<String>)> {
    if let Some(pos) = s.find(".[") {
        ...
        let pos = s
            .rfind('.')
            .ok_or_else(|| EfmError::new(line, format!("referencia inválida: '{s}'")))?;
        let left = &s[..pos];
...
fn parse_index(node: &Node) -> Result<Index> {
    ...
    let map = obj
        .as_object()
        .ok_or_else(|| EfmError::new(node.line, "se esperaba un objeto de índice"))?;
...
fn parse_trigger(table: &Table, node: &Node) -> Result<Trigger> {
    ...
    let timing = node
        ...
        .transpose()?
        .ok_or_else(|| EfmError::new(node.line, "trigger sin 'timing'"))?;
...
fn parse_seed_row(node: &Node) -> Result<BTreeMap<String, Value>> {
    ...
    let map = v
        .as_object()
        .ok_or_else(|| EfmError::new(node.line, "fila de seed inválida"))?;
...
fn parse_action(s: &str, line: usize) -> Result<ReferentialAction> {
    Ok(match s {
        ...
        other => return Err(EfmError::new(line, format!("onDelete inválido: '{other}'"))),
    })
}
...
fn parse_timing(s: &str, line: usize) -> Result<TriggerTiming> {
    Ok(match s.trim() {
        ...
        other => return Err(EfmError::new(line, format!("timing inválido: '{other}'"))),
    })
}
...
fn parse_event(s: &str, line: usize) -> Result<TriggerEvent> {
    Ok(match s.trim() {
        ...
        other => return Err(EfmError::new(line, format!("evento inválido: '{other}'"))),
    })
}
