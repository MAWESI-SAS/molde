//! Errores del lenguaje EFM, con diagnósticos legibles: línea, columna y un
...
mod tests {
    ...
    fn with_source_localiza_columna_del_token_citado() {
        let src = "Customer:\n  fields:\n    Email: string uniq\n";
        let e = EfmError::new(3, "faceta desconocida: 'uniq'").with_source(src);
    ...
    fn with_source_sin_token_apunta_al_primer_no_blanco() {
        let src = "  Customer\n";
        let e = EfmError::new(1, "sección desconocida").with_source(src);
        assert_eq!(e.column, Some(3));
    }

    #[test]
    fn in_file_prefija_el_nombre() {
        let e = EfmError::new(2, "x").in_file("Customer.model");
    ...
    fn with_source_es_idempotente() {
        let src = "uno\ndos\n";
        let e = EfmError::new(1, "err").with_source(src);
