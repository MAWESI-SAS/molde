...
//!   cargo test -p efrust-scaffold --test roundtrip_sqlserver -- --ignored --nocapture
//! ```

use efrust_core::diff::diff;
use efrust_core::model::DatabaseModel;
use efrust_providers::{Provider, SqlGenerator, SqlServerGenerator};
use tiberius::Client;
...
async fn round_trip_sqlserver_computed_persisted() {
    ...
    let mut src_model = efrust_scaffold::reader::read_model(&src, Provider::SqlServer, Some("dbo"))
        .await
        .expect("leyendo SRC");
    src_model.normalize();
    ...
    let mut dst_model = efrust_scaffold::reader::read_model(&dst, Provider::SqlServer, Some("dbo"))
        .await
        .expect("leyendo DST");
    dst_model.normalize();
