...
//!   cargo test -p efrust-scaffold --test roundtrip_mysql -- --ignored --nocapture
//! ```

use efrust_core::diff::diff;
use efrust_core::model::DatabaseModel;
use efrust_providers::{MySqlGenerator, Provider, SqlGenerator};
...
async fn round_trip_mysql_busqueda() {
    ...
    let mut src_model = efrust_scaffold::reader::read_model(&src, Provider::MySql, None)
        .await
        .expect("leyendo SRC");
    src_model.normalize();
    ...
    let mut dst_model = efrust_scaffold::reader::read_model(&dst, Provider::MySql, None)
        .await
        .expect("leyendo DST");
    dst_model.normalize();
