//! efrust-lsp — language server del lenguaje de modelos EFM (`.model`).
//!
//! Reutiliza `efrust-lang` para parsear/formatear/inspeccionar. Provee:
...
impl Backend {
    ...
    async fn validate(&self, uri: Url) {
        ...
        let diags = match efrust_lang::format_model(&name, &text) {
            Ok(_) => Vec::new(),
...
impl LanguageServer for Backend {
    ...
    async fn initialized(&self, _: InitializedParams) {
        ...
        self.client
            .log_message(
                MessageType::INFO,
                format!("efrust-lsp: {n} entidades indexadas"),
            )
            .await;
    }
    ...
    async fn document_symbol(
        ...
        let Ok(items) = efrust_lang::outline(&content) else {
            return Ok(None);
        };
    ...
    async fn formatting(&self, params: DocumentFormattingParams) -> Result<Option<Vec<TextEdit>>> {
        ...
        let name = text::uri_file_name(&uri);
        let Ok(formatted) = efrust_lang::format_model(&name, &content) else {
