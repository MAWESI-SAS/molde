...
/// `ON DELETE` clause for a referential action (empty for `NoAction`).
...
impl PostgresGenerator {
    ...
    fn column_def(&self, column: &Column) -> Result<String, ProviderError> {
        ...
        // Computed/generated column (e.g. tsvector STORED). Excludes default.
    ...
    fn store_type_for(&self, column: &Column) -> Result<String, ProviderError> {
        ...
        let mapped = match clr {
            ...
            "System.Decimal" => match (column.precision, column.scale) {
                ...
                // `numeric` without precision = arbitrary precision (valid in PG).
                _ => "numeric",
            },
    ...
    fn emit(&self, op: &Operation) -> Result<Vec<String>, ProviderError> {
        let sql = match op {
            ...
            Operation::DropIndex { schema, name, .. } => {
                // In Postgres indexes live in the schema of their table.
                vec![format!(
                    "DROP INDEX {};",
                    self.qualified(schema.as_deref(), name)
                )]
            ...
            // Functions and triggers are preserved as raw SQL (not modelable by
            // EF). The definition is already valid Postgres DDL (pg_get_*def).
...
impl PostgresGenerator {
    ...
    fn create_index(&self, schema: Option<&str>, table: &str, index: &Index) -> String {
        ...
        // Access method (gin/gist/hnsw/ivfflat). Empty = btree by default.
        let using = index
            .method
            .as_deref()
            .filter(|m| !m.is_empty())
            ...
            .unwrap_or_default();
        // Optional operator class per key (pgvector: vector_cosine_ops, etc.).
        ...
        // Index by expression (full-text/functional) vs by columns.
