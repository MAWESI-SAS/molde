...
pub fn run(args: SyncArgs) -> Result<()> {
    ...
    let engine = engine_for(&source).context(
        "unrecognized source database (use a postgres://, mysql://, sqlite: URL \
         or a SQL Server ADO string)",
    )?;
...
/// Wrap the additive body in a portable, atomic script file with a header. The
/// transaction wrapper is engine-specific (`wrap_script`).
fn script_file(
    engine: &dyn molde_sync::SyncEngine,
    body: &str,
    d: &DiffResult,
    source: &str,
    target: &str,
    ...
    s.push('\n');
    s.push_str(&engine.wrap_script(body));
    s
}
