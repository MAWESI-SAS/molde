...
Herramienta de **esquema de base de datos en Rust** construida sobre **EFM**, un
lenguaje de modelos propio, declarativo y legible (estilo TOON/YAML). efrust hace
**scaffold** (BD → modelos), genera **migraciones** (modelo → diff) y las
**aplica** sobre 4 motores. Sin .NET, sin C#: todo en Rust.

> efrust gestiona el **esquema** (modelos, migraciones, aplicación). El acceso a
> datos en runtime (consultas de tu aplicación) queda fuera de su alcance.

## El lenguaje EFM (`.model`)
...
Especificación completa: [`docs/efm-language-spec.md`](docs/efm-language-spec.md).
...
Todo pivota sobre un **IR** común (`efrust_core::DatabaseModel`): el lenguaje es su
...
├── efrust-lang         lenguaje EFM: parser/emitter (.model ↔ IR)
...
├── efrust-scaffold     lectura de esquema (BD → IR) + emisión de .model
└── efrust-design       autoría de migraciones (diff contra snapshot)
```
...
efrust scaffold --connection "$DATABASE_URL" --output-dir models

# 2. Model-first: crear una migración a partir de los .model (diff vs snapshot).
efrust migrations add InitialCreate           # lee models/, escribe migrations/
efrust migrations list
efrust migrations remove

# 3. Aplicar / revertir migraciones contra la BD.
efrust database update --connection "$DATABASE_URL"
efrust database update --connection "$DATABASE_URL" --target 0            # revertir todo
efrust database update --connection "$DATABASE_URL" --target InitialCreate
```
...
migrations/             # versionado en git
  snapshot.json         # estado previo (gestionado por efrust)
